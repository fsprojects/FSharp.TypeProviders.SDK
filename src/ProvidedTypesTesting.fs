...
type internal Testing() =
    ...
    static member FormatProvidedType (tp: TypeProviderForNamespaces, t: ProvidedTypeDefinition, ?signatureOnly, ?ignoreOutput, ?maxDepth, ?maxWidth, ?useQualifiedNames) =
        ...
        let printMember (memberInfo: MemberInfo) =
            ...
            let getMethodBody (m: ProvidedMethod) =
                ...
                (tp :> ITypeProvider).GetInvokerExpression(m, List.toArray vs)
            ...
            let getConstructorBody (c: ProvidedConstructor) =
                let vs = 
                    ...
                    |> List.map (Var.Global >> Expr.Var)
                (tp :> ITypeProvider).GetInvokerExpression(c, List.toArray vs)
